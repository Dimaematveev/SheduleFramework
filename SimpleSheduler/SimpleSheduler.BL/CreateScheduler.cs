...
namespace SimpleSheduler.BL
{
    public class CreateScheduler
    {
        ...
        private List<Curriculum> AddPairIntoScheduler(Classroom[] Classrooms)
        {
            ...
            for (int cu = 0; cu < FillingClassrooms.Length; cu++)
            {
                ...
                for (int cl = NumClassroom; cl < Classrooms.Length; cl++)
                {
                    ...
                    List<PossibleFilling> busyPairs = new List<PossibleFilling>();
                    busyPairs.Add( fillingTeacher[cu]);
                    busyPairs.Add(fillingGroup[cu]);
                    busyPairs.Add(fillingClassroom[cu]);
                    bool check = true;
                    //Условия что в этот день в эту пару Преподаватель группа и классная комната не заняты
                   
                    {
                        check = check && (fillingTeacher[cu].BusyPair == null);
                        check = check && (fillingGroup[cu].BusyPair == null);
                        check = check && (fillingClassroom[cu].BusyPair == null);
                    }
        ...
        private bool CheckAndAddPairIntoSchedule(
            List<PossibleFilling> busyPairs,
            Teacher teacher,
            Subject subject,
            Group group,
            Classroom classroom)
        {
            ...
            {
                check = check && (busyPair1.BusyPair == null);
            }
            //1. Если в этот день свободна и группа и преподаватель и аудитория
            if (check)
            {
